		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					bSupportDragAndDropAPI = false;// 찾아보기
				}else{
					bSupportDragAndDropAPI = false;	// drag&drop : false 설정
				}
			} else {
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/MyPrice/SE2/photo_uploader/popup/file_uploader.jsp',	//샘플 URL입니다.
 			//sUrl  : '/file_uploader.do',	//샘플 URL입니다.
 	        sCallback : '/MyPrice/SE2/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
